...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        # self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, groups=inplanes, bias=False)
        # self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        # self.relu = nn.ReLU(inplace=True)
        # self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, groups=inplanes, bias=False)
...
class BHRNet(nn.Module):
    def __init__(self, base_channel: int = 64, num_joints: int = 20):
        super().__init__()
        # Stem
        self.num_joints = num_joints
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        # 更改输出热图大小，只需修改下面conv2的stride即可
...
class BHRNet98(nn.Module):
    def __init__(self, base_channel: int = 64, num_joints: int = 32):
        super().__init__()
        # Stem
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
...
if __name__ == "__main__":
    ...
    model = HighResolutionNet(num_joints=98)
    output = model(input)
    print(output.size())
